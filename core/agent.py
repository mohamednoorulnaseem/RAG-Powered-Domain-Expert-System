...
class RagasEvaluator:
    ...
    def __init__(self, groq_key: str):
        ...
        self.groq_llm = ChatGroq(model="llama-3.1-8b-instant", groq_api_key=groq_key, temperature=0.0)
    ...
    def evaluate_sample(self, question: str, answer: str, contexts: List[str]) -> Dict[str, float]:
...
class SelfCorrectingRAGAgent:
    ...
    def node_retrieve(self, state: AgentState) -> Dict[str, Any]:
        ...
        logger.info(f"Vector store has {len(self.query_engine.vector_store.entries)} entries.")
    ...
    def node_rerank(self, state: AgentState) -> Dict[str, Any]:
        ...
        logger.info(f"--- Node [rerank]: Reranking {len(state['retrieved_docs'])} candidates ---")
...
Rewritten search query:"""
    ...
    def route_after_evaluation(self, state: AgentState) -> str:
        ...
        is_satisfactory = (state["faithfulness_score"] >= 0.80) and (state["answer_relevance_score"] >= 0.80)
        ...
        if is_satisfactory or max_attempts_reached:
            if max_attempts_reached and not is_satisfactory:
                logger.warning("Max generation attempts (3) reached. Exiting agent loop with best effort response.")
            else:
                logger.success("Agent evaluation criteria met successfully.")
            return "end"
            
        logger.info("Evaluation criteria not met. Routing to query rewriting node for another attempt.")
        return "continue"
