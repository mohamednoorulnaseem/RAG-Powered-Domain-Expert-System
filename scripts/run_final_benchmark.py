...
def run_evaluation():
    ...
    output_data = {
        ...
        "detailed_results": [
            ...
            for q, ans, gt, lat, att in zip(
                questions, answers, ground_truths, latencies, attempts_counts
            )
        ],
    }
    ...
    print("=" * 70)
    print(
        f"{'Metric':<25} | {'Baseline':<10} | {'Reranked':<10} | {'Agentic Loop':<12}"
    )
    print("-" * 70)
    print(
        f"{'Faithfulness':<25} | {'0.6263':<10} | {'0.6663':<10} | {avg_faithfulness:<12.4f}"
    )
    print(
        f"{'Answer Relevancy':<25} | {'0.7239':<10} | {'0.7231':<10} | {avg_relevancy:<12.4f}"
    )
    print(
        f"{'Avg Latency (ms)':<25} | {'~1500':<10} | {'~2200':<10} | {avg_latency:<12.1f}"
    )
    print(f"{'Avg Attempts':<25} | {'1.00':<10} | {'1.00':<10} | {avg_attempts:<12.2f}")
    print("=" * 70)
    print(
        "Comparative report successfully compiled and saved to docs/metrics/agent.json."
    )
